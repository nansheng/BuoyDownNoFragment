...
public class MessageReplyReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (BuoyDownMessagingService.REPLY_ACTION.equals(intent.getAction())) {
            int conversationId = intent.getIntExtra(BuoyDownMessagingService.CONVERSATION_ID, -1);
    ...
    private CharSequence getMessageText(Intent intent) {
        ...
        if (remoteInput != null) {
            return remoteInput.getCharSequence(BuoyDownMessagingService.EXTRA_VOICE_REPLY);
        }
        return null;
    }
}
